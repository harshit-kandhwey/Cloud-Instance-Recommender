...
(async () => {
  ...
  check("azure returns display names", azure.getAllAvailableRegionKeys().includes("East US"));
  check("gcp returns dash keys", gcp.getAllAvailableRegionKeys().includes("us-central1"));
  ...
  check("aws instances parsed", (aws.instanceData["us-east-1"] || []).length > 100,
  ...
  check("azure instances parsed", (azure.instanceData["East US"] || []).length > 100);

  await gcp.loadInstanceData(new Set(["us-central1-a"]));
  check("gcp zone suffix stripped → us_central1", typeof ctx.us_central1 === "object");
  check("gcp instances parsed", (gcp.instanceData["us-central1-a"] || []).length > 50);

  console.log("[unknown region → fallback, no 404]");
  const before = requestedSrcs.length;
  await aws.loadRegionData("xx-fake-9");
  check("no script request for unknown region", requestedSrcs.length === before,
    `requested: ${requestedSrcs.slice(before).join(", ")}`);
  check("fallback data parsed", (aws.instanceData["xx-fake-9"] || []).length > 0);
  ...
  check("3 concurrent requests → 1 script tag", newReqs.length === 1,
    `got ${newReqs.length}`);
