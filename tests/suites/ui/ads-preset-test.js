...
}
{
  ...
  const usageIndex = ADS_HEADERS.indexOf("RAM.UsedSizeInMB.Avg");
  check("RAM usage header exists", usageIndex >= 0, `index=${usageIndex}`);
  if (usageIndex >= 0) withoutUsage[usageIndex] = "";
  const { ctx } = buildContext();
