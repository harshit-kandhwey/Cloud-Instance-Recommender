 ...
 * written by fetch-vantage) into the shipped manifest src/providers/{provider}/{provider}-data.js
 * plus per-region files (src/providers/{provider}/regions/<regionKey>.js). This is the ONLY step
 * of the refresh that writes into the shipped src/providers/ tree.
 *
 ...
 *   1. Run scripts/data/fetch-vantage.js — it writes .refresh-cache/{provider}-monolith.js
 *      (or drop an equivalent monolith there by hand).
 *   2. Run: node scripts/data/split-data.js
...
function splitProvider({ name, prefix }, root = ROOT) {
  ...
  const dataPath = path.join(root, "src", "providers", name, `${name}-data.js`);
  ...
  const manifest =
    headerLines.join("\n") +
    (headerLines.length ? "\n" : "") +
    `// Manifest generated by scripts/data/split-data.js — per-region prices live in src/providers/${name}/regions/\n` +
  ...
  const files = declaredKeys.map((key) => ({
    ...
    fileContent:
      ...
      `// Auto-generated by scripts/data/split-data.js — do not edit by hand\n` +
      `// Specs for these types live in src/providers/${name}/${name}-data.js as ${prefix}_SPECS.${SERVICE}\n` +
      `window.${key} = {\n` +
  ...
  const regionsDir = path.join(root, "src", "providers", name, "regions");
  ...
  console.log(
    `[${name}] split ${declaredKeys.length} regions into src/providers/${name}/regions/ ` +
...
function verifyRoundTrip({
  ...
  const check = evaluate(manifest, `src/providers/${name}/${name}-data.js`);
  for (const { key, fileContent } of files) {
...
function main() {
  ...
  // `node scripts/data/split-data.js | tee split.log` and then greps that log for
