...
// implemented, not why.
//
//   node scripts/testing/build-coverage-inventory.js          # write the report
//   node scripts/testing/build-coverage-inventory.js --check   # read-only gate; exit 1 on
//                                                     #   a behavioral gap
//
...
// Everything under src/ except per-region data (generated by
// scripts/data/split-data.js) and the monolithic *-data.js manifests, plus
// js/pwa-register.js — the one first-party file the js/ -> src/ move (3.16
// tail) deliberately left outside src/ (see CLAUDE.md's environment notes).
// js/vendor/ stays excluded implicitly: it's not under src/ at all now, so
// there's nothing to skip by name the way "vendor" once had to be.
function listJs(dir, out = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      if (entry.name === "regions") continue;
      listJs(full, out);
