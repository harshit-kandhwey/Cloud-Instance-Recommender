...
Everything runs client-side. The Overview tab's KPI row includes a relative-only "Optimized ranks ~X% lower than Like-to-Like" figure per provider when both were generated — no absolute dollar amount, and the exported Executive Excel workbook carries none either.
...
The in-browser **results preview** table includes sortable columns, a live search filter, a per-row copy button, vCPU diff highlighting (green = smaller / amber = larger vs Like-to-Like), a **fit/headroom flag** (a ▲% beside a like-for-like match, showing how far the chosen instance over-provisions its worst axis versus the requested size — the discrete-sizing and ratio-mismatch waste), and a stats bar showing match rate, rules fired, a relative cost figure when both recommendation types were generated, and data freshness date. Rows that got no recommendation from any provider can be exported separately via the **CSV ▾** menu's **No-Match Rows** item for fix-and-re-upload remediation. The grid downloads primarily as a styled Excel workbook (**📊 Download Results (Excel)**, with a sheet per alternative strategy) — with the flat CSVs behind the **CSV ▾** checklist — and any two runs can be pinned and diffed with **Scenario comparison** to see exactly what a filter change did — including a one-click **Export comparison CSV** that writes the configuration changes and the changed recommendation rows to a single file. A **🖨️ Print Report** button opens a print-ready one-page executive summary (headline stats plus the match-rate, family, relative cost, and before → after charts) to print or save as PDF.

> **No absolute pricing appears anywhere in output — a relative estimate does, since 3.17.** Cloud pricing depends on region, OS, discounts, Reserved Instances, Savings Plans, and enterprise agreements, so a static dollar figure in a CSV would be misleading within weeks — that has not changed, and no export, column, or sheet carries one. What the stats bar, the App Portfolio Overview, and the Executive Print Report now show instead is a single aggregate, per-provider **relative** figure — "Optimized ranks ~X% lower than Like-to-Like" — computed from the same internal ranking data the engine always used, shown only when both recommendation types were generated, and always paired with the data-freshness date and a reminder to use the provider's pricing calculator for an actual quote.
